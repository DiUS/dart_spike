...
$$.TodoView = {"":"Object;todo",
 renderTodoHtml$0: function() {
  var t1, t2, t3, el;
  t1 = this.todo;
  t2 = "<div id=\"todo-" + $.S(t1.id) + "\" class=\"row todo ";
  t3 = t2 + (t1.complete == null ? "" : "completed") + "\" data-id=\"" + $.S(t1.id) + "\">" + "  <input class=\"todo-check\" type=\"checkbox\" ";
  t2 = t3 + (!(t1.complete == null) ? "checked=\"checked\"" : "") + ">" + "  <div class=\"todoText\">" + $.S(t1.todoText) + "</div>" + "  <div class=\"completedDate\">";
  t1 = t1.complete;
  el = $.Element_Element$html(t2 + $.S(t1 == null ? "" : $.getInterceptor(t1).toString$0(t1)) + "</div>" + "</div>");
  t1 = el.get$on().get$click();
  $.getInterceptor$JSArray(t1).add$1(t1, this.get$handleClick());
  return el;
},
 handleClick$1: function(e) {
  ...
  if (!(t1.complete == null))
    ...
    t1.complete = $.Date_Date$now();
  data = $.JSON_stringify(t1);
  request = $.HttpRequest_HttpRequest();
  request.open$2("PUT", "/todos/" + $.S(t1.id) + "?payload=" + $.S(data));
  request.send$1(data);
  this.updateView$0();
},
 get$handleClick: function() {
  return new $.BoundClosure(this, 'handleClick$1');
},
 updateView$0: function() {
  var t1, todoElement;
  t1 = this.todo;
  todoElement = $.query("#todo-" + $.S(t1.id));
  todoElement.get$classes().toggle$1("completed");
  t1 = t1.complete;
  t1 = t1 == null ? "" : $.getInterceptor(t1).toString$0(t1);
  todoElement.query$1(".completedDate").set$text(t1);
}
};
...
$$.Todo = {"":"Object;id>,todoText>,complete=",
 isValid$0: function() {
  var t1 = this.todoText;
 ...
 toJson$0: function() {
  ...
  t2 = this.todoText;
  t3 = this.complete;
  return $.makeLiteralMap(["id", t1, "todoText", t2, "complete", $.getInterceptor(t3).toString$0(t3)]);
},
 toString$0: function() {
  var t1, t2, t3, t4;
  t1 = this.id;
  t2 = this.todoText;
  t3 = this.complete;
  t4 = $.makeLiteralMap(["id", t1, "todoText", t2, "complete", $.getInterceptor(t3).toString$0(t3)]);
  return $.getInterceptor(t4).toString$0(t4);
},
 Todo$fromJson$1: function(jsonMap) {
  if ($.eqB(this.complete, "null"))
    this.complete = null;
  else
    this.complete = $.Date_Date$fromString(this.complete);
}
};
...
$$.loadList__anon = {"":"Closure;",
 call$1: function(todoJson) {
  var view, t1;
  view = $.TodoView$($.Todo$fromJson(todoJson));
  t1 = $.query("#todo-list").get$children();
  $.getInterceptor$JSArray(t1).add$1(t1, view.renderTodoHtml$0());
}
};
...
$$.handleSubmit_anon = {"":"Closure;input_0,request_1",
 call$1: function(_) {
  var t1, alerts, alert$, t2, todoElement, button;
  t1 = this.request_1;
  if ($.eqB(t1.get$readyState(), 4)) {
    alerts = $.query("#alerts");
    if ($.eqB(t1.get$status(), 200)) {
      alert$ = $.Element_Element$html("<div class=\"alert alert-success\">  <button type=\"button\" class=\"close\" data-dismiss=\"alert\">&times;</button>  Todo created OK.</div>");
      t2 = alerts.get$children();
      $.getInterceptor$JSArray(t2).add$1(t2, alert$);
      this.input_0.set$value("");
      todoElement = $.TodoView$($.Todo$fromJson($.JSON_parse(t1.get$responseText()))).renderTodoHtml$0();
      t2 = $.query("#todo-list").get$children();
      $.getInterceptor$JSArray(t2).add$1(t2, todoElement);
    } else {
      alert$ = $.Element_Element$tag("div");
      $.indexSet(alert$.get$attributes(), "class", "alert alert-error");
      alert$.set$text("Failed to save Todo!");
      button = $.Element_Element$tag("button");
      $.indexSet(button.get$attributes(), "type", "button");
      $.indexSet(button.get$attributes(), "class", "close");
      $.indexSet(button.get$attributes(), "data-dismiss", "alert");
      t1 = alerts.get$children();
      $.getInterceptor$JSArray(t1).add$1(t1, alert$);
      t1 = alert$.get$children();
      $.getInterceptor$JSArray(t1).add$1(t1, button);
      t1 = button.get$children();
      $.getInterceptor$JSArray(t1).add$1(t1, $.Element_Element$html("<span>&times;</span>"));
    }
  }
}
};
...
$.TodoView_fromElement = function(element) {
  var todo, t1, complete;
  todo = $.Todo$(element.query$1(".todoText").get$text());
  t1 = element.get$dataAttributes();
  todo.id = $.getInterceptor$JSStringJSArray(t1).operator$index$1(t1, "id");
  complete = element.query$1(".completedDate").get$text();
  if ($.getInterceptor$JSStringJSArray(complete).get$isEmpty(complete) !== true)
    todo.complete = $.Date_Date$fromString(complete);
  return todo;
};

$.handleSubmit = function(e) {
  var input, todo, data, request, t1;
  e.preventDefault$0();
  input = $.query("#new-todo input");
  todo = $.Todo$(input.get$value());
  if (todo.isValid$0() === true) {
    data = $.JSON_stringify(todo);
    request = $.HttpRequest_HttpRequest();
    t1 = request.get$on().get$readyStateChange();
    $.getInterceptor$JSArray(t1).add$1(t1, new $.handleSubmit_anon(input, request));
    request.open$2("POST", "/todos?payload=" + $.S(data));
    request.send$1(data);
  }
};

$.loadList = function() {
  $.HttpRequest_HttpRequest$get("/todos", new $.loadList_anon());
};

$.handleClearCompleted = function(e) {
  var t1, t2, todo;
  for (t1 = $.query("#todo-list").get$children(), t1 = $.getInterceptor$JSArray(t1).iterator$0(t1); t1.get$hasNext() === true;) {
    t2 = t1.next$0();
    todo = $.TodoView_fromElement(t2);
    if (!(todo.complete == null)) {
      $.JSON_stringify(todo);
      $.HttpRequest_HttpRequest().open$2("DELETE", "/todos/" + $.S(todo.id));
      t2.remove$0();
    }
  }
};
...
$.main = function() {
  var t1 = $.query("#new-todo").get$on().get$submit();
  $.getInterceptor$JSArray(t1).add$1(t1, $.handleSubmit);
  t1 = $.query("#delete-button").get$on().get$click();
  $.getInterceptor$JSArray(t1).add$1(t1, $.handleClearCompleted);
  $.loadList();
};
...
$.Primitives_valueFromDecomposedDate = function(years, month, day, hours, minutes, seconds, milliseconds, isUtc) {
  ...
  if ($.CONSTANT4.get$isNaN(value) === true || value < -8640000000000000 || value > 8640000000000000)
    throw $.$$throw($.ArgumentError$(null));
...
$._DateImpl__DateImpl$fromString = function(formattedString) {
  ...
  if (!(match == null)) {
    ...
    t3 = $.getInterceptor$JSStringJSArray(match);
    years = $.Primitives_parseInt(t3.operator$index$1(match, 1));
    month = $.Primitives_parseInt(t3.operator$index$1(match, 2));
    day = $.Primitives_parseInt(t3.operator$index$1(match, 3));
    hour = t1.call$1(t3.operator$index$1(match, 4));
    minute = t1.call$1(t3.operator$index$1(match, 5));
    second = t1.call$1(t3.operator$index$1(match, 6));
    t1 = $.mul(t2.call$1(t3.operator$index$1(match, 7)), 1000);
...
$.Todo$ = function(todoText) {
  return new $.Todo(null, todoText, null);
};

$.Todo$fromJson = function(jsonMap) {
  var t1, t2;
  t1 = $.getInterceptor$JSStringJSArray(jsonMap);
  t2 = new $.Todo(t1.operator$index$1(jsonMap, "id"), t1.operator$index$1(jsonMap, "todoText"), t1.operator$index$1(jsonMap, "complete"));
  t2.Todo$fromJson$1(jsonMap);
  return t2;
};

$.handleSubmit.call$1 = $.handleSubmit;
$.handleSubmit.$name = "handleSubmit";
$.handleClearCompleted.call$1 = $.handleClearCompleted;
$.handleClearCompleted.$name = "handleClearCompleted";
...
$.$defineNativeClass('HTMLFormElement', {"":"length>",
 ...
 get$submit: function() {
  return new $.BoundClosure0(this, 'submit$0');
}
});
...
//@ sourceMappingURL=todo_page.dart.js.map